...
__kernel void highLifeEngine(__global unsigned char *map_in, __global unsigned char *map_out, __write_only image2d_t image, int width, int height)
{
    ...
    if(get(map_in, x, y, width, height, gw, gh) == 1 && !(cell_count == 3 || cell_count == 2))
        map_out[ y * gw + x ] = 0;
    else if(get(map_in, x, y, width, height, gw, gh) == 0 && (cell_count == 3 || cell_count == 6))
        map_out[ y * gw + x ] = 1;
    else
